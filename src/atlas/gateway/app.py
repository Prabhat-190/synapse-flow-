...
from atlas.gateway.security import check_rate_limit, screen_prompt_injection, verify_api_key
from atlas.infra.database import get_trace_events_since, init_db, load_trace

logger = structlog.get_logger()

REQUEST_COUNT = Counter("atlas_requests_total", "Total API requests", ["endpoint", "status"])
REQUEST_LATENCY = Histogram("atlas_request_latency_seconds", "Request latency", ["endpoint"])
PIPELINE_RUNS = Counter("atlas_pipeline_runs_total", "Pipeline executions", ["status"])
...
app = FastAPI(
    ...
    description="Adaptive Typed LangGraph Agent System — Multi-Agent LLM Orchestration",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def query(
    ...
    if req.async_mode:
        from atlas.infra.celery_app import run_pipeline_task
...
async def stream_run(run_id: str, api_key: str = Depends(verify_api_key)):
    ...
    async def event_generator():
        from atlas.infra.redis_bus import subscribe_events
